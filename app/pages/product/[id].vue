  ...
  <div class="mx-auto max-w-5xl px-4 py-8">
    ...
    <div class="breadcrumbs mb-6 text-sm">
      <ul>
        ...
        <li class="max-w-xs truncate">{{ product?.name }}</li>
      </ul>
    </div>
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
      ...
      <div>
        <!-- 主圖 -->
        <div class="bg-base-200 mb-3 aspect-square overflow-hidden rounded-xl">
          <img
            v-if="activeImage"
            :src="activeImage"
            :alt="product?.name"
          ...
          <div
            v-else
            class="flex h-full w-full items-center justify-center text-6xl opacity-20"
          >
            📦
          </div>
        </div>
        ...
        <div v-if="images.length > 1" class="flex flex-wrap gap-2">
          <button
            ...
            :class="
              idx === activeImageIdx ? 'border-primary' : 'border-transparent'
            "
      ...
      <div class="flex flex-col gap-4">
        <h1 class="text-2xl leading-snug font-bold">{{ product?.name }}</h1>
        ...
        >
          ...
          <NuxtLink :to="`/shop/${shop.id}`" class="link link-hover">{{
            shop.name
          }}</NuxtLink>
        </div>
        ...
        <div v-if="specs.length" class="overflow-x-auto">
          <table class="table-sm bg-base-100 table rounded-xl">
            <thead>
              <tr>
                <th>規格</th>
                <th>價格</th>
              ...
              <tr v-for="spec in specs" :key="spec.id">
                ...
                <td>
                  <template v-if="spec.sale_price > 0">
                    ...
                    >
                    <span
                      class="text-base-content/40 ml-1 text-xs line-through"
        ...
        <!-- ── 購買按鈕 ── -->
        <button class="btn btn-primary btn-lg mt-2 w-full" @click="handleBuy">
        ...
        <div class="text-base-content/40 flex items-center gap-1 text-xs">
          <Icon name="heroicons:fire" class="h-4 w-4 text-orange-400" />
    ...
    <div v-if="product?.description" class="mt-10">
      <h2 class="mb-4 text-lg font-bold">商品說明</h2>
      <div
